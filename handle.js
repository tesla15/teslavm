    ...
    if (useefi) {
        if (finalaccel == "hax") {
            ...
            if (cores > 2) {
                ...
                command = command_base + cpu_type + `.exe -device AC97 -usbdevice tablet -display gtk,gl=on -machine q35 -cpu SandyBridge, -m ${ram}G -smp ${cores} -boot ${border} -vga ${vgaac} -accel ${finalaccel} -bios ../ovmf.fd`;
            }
        } else {
            command = command_base + cpu_type + `.exe -device AC97 -usbdevice tablet -display gtk,gl=on -machine q35 -cpu SandyBridge -m ${ram}G -smp ${cores} -boot ${border} -vga ${vgaac} -accel ${finalaccel} -bios ../ovmf.fd`;
        ...
        console.log("no uefi")
        command = command_base + cpu_type + `.exe -device AC97 -usbdevice tablet -display gtk,gl=on -machine q35 -cpu SandyBridge -m ${ram}G -smp ${cores} -boot ${border} -vga ${vgaac} -accel ${finalaccel}`;
    }
